...
class Tarea extends Base {
  ...
  public function setSpecifics($post) {
    ...
    if(isset($post['usuarios'])) {
      ...
      $query = "DELETE FROM tareas_usuarios WHERE id_tareas='".$this->id."'";
      $this->runQuery($query);
      ...
      if(is_array($post['usuarios'])) {
        foreach($post['usuarios'] as $id_usuario) {
          if(is_numeric($id_usuario) && $id_usuario > 0) {
            $tarea_usuario = new TareaUsuario();
            $tarea_usuario->id_tareas = $this->id;
    ...
    if(isset($post['etiquetas'])) {
      ...
      $query = "DELETE FROM tareas_etiquetas WHERE id_tareas='".$this->id."'";
      $this->runQuery($query);
      ...
      if(is_array($post['etiquetas'])) {
        foreach($post['etiquetas'] as $id_etiqueta) {
          if(is_numeric($id_etiqueta) && $id_etiqueta > 0) {
            $tarea_etiqueta = new TareaEtiqueta();
            $tarea_etiqueta->id_tareas = $this->id;
            $tarea_etiqueta->id_etiquetas = $id_etiqueta;
            $tarea_etiqueta->save();
          }
        }
      }
  ...
  public function getUsuarios() {
    $query = "SELECT id_usuarios FROM tareas_usuarios WHERE id_tareas='".$this->id."'";
  ...
  public function getEtiquetas() {
    $query = "SELECT id_etiquetas FROM tareas_etiquetas WHERE id_tareas='".$this->id."'";
    $result = $this->fetchQuery($query);
    $etiquetas = array();
    foreach($result as $row) {
      $etiquetas[] = (int)$row['id_etiquetas'];
    }
    return $etiquetas;
  }

  public function getCantidadArchivos() {
    $query = "SELECT COUNT(*) as total FROM media_tareas WHERE id_tareas='".$this->id."'";
