    ...
    <style>
        ...
        <div class="row">
          <div class="col-2 px-2">
            ...
            <button class="btn btn-sm btn-outline-primary mb-3" data-bs-toggle="modal" data-bs-target="#llenar-barriles-modal" id="llenar-barriles-btn"> 
                  Llenar Barril
              </button>
          ...
          <div class="col-5 px-2">
            ...
            <br/>
            <div class="dragger-container pt-1 shadow" id="container1" data-dropzone-role="pedidos" style="height: 80vh; overflow: scroll">
              <a href="./?s=nuevo-pedidos" class="btn btn-sm btn-outline-primary mb-3">
                + Crear Pedido
              </a>
              <?php
              ...
              foreach($pedidos as $pedido) {
                  ...
                  $productos = PedidoProducto::getAll("WHERE id_pedidos='".$pedido->id."' AND estado='Solicitado'");
                ...
                ?>
                  ...
                  foreach($productos as $pp) {
                      ...
                      ?>
                      <div class="card dragger-card shadow border" draggable="true" data-id-pedidos-productos="<?= $pp->id; ?>" data-id-barriles="0" data-id-despachos-productos="0" data-id-despachos="0" data-id-usuarios-repartidor="0">
                          <div class="card-body">
        ...
        <div class="col-5 px-2" style="height: 80vh; overflow: scroll">
            <div class="d-flex justify-content-between">
              <div class="w-100 text-center">
              <h2>Despachos</h2>
            </div>
            ...
            <br/>
            <div class="repartos-dragger-container shadow" style="height: 80vh; overflow: scroll">
              <?php
                  ...
                  <?php
                    foreach($despachos as $d) {
                      ...
                      foreach($dps as $dp) {
                        ...
                        $tiempo_pasado = floor(($date1 - $date2) / (60 * 60 * 24));
                        ?>
                        <div class="card dragger-card shadow border" draggable="true" data-id-pedidos-productos="<?= $dp->id_pedidos_productos; ?>" data-id-barriles="<?= $barrili->id; ?>" data-id-despachos-productos="<?= $dp->id; ?>"  data-id-despachos="<?= $d->id; ?>" data-id-usuarios-repartidor="<?= $repartidor->id; ?>">
                          <div class="card-body">
                              <div class="d-flex justify-content-between">
                                  <div>
                                      <div class="fw-bold fs-4">
...
$(document).ready(function() {
    ...
    getCentralDeDespacho();
    
});



function  getCentralDeDespacho() {

    var url = './ajax/ajax_getCentralDeDespacho.php';
    var data = {};
...
function armarBarrilesEnPlanta() {
    var html = '';
    barriles_en_planta_2.forEach((bep) => {
        html += `
            <tr data-idbarriles="${bep.id}">
                <td>
                    ${bep.codigo}
                </td>
                <td>
                ...
                <td>
                    ${bep.litros_cargados}L
                </td>
            </tr>
        `;
    });
    $('#barriles-en-planta-id').html(html);
}

function armarBatchesActivosMaduracion() {
    var html = '';
    batches_activos_maduracion.forEach((bam) => {
        html += `
            <option value="${bam.id}">${bam.activo_codigo} (${bam.activo_litraje}L - ${bam.receta_nombre})</option>
        `;
    });
    $('#llenar-barriles_id_batches_activos-select').html(html);
}

function armarBarrilesDisponiblesParaCarga() {
    var html = '';
    barriles_disponibles.forEach((bd) => {
        html += `
            <option value="${bd.id}">${bd.codigo} (${bd.litraje}L)</option>
        `;
    });
    $('#llenar-barriles_id_barriles-select').html(html);
}
...
function getDraggedCards() {
    ...
    const draggerContainers = document.querySelectorAll(".dragger-container-2");
        ...
        if(container.getAttribute('data-id-usuarios-repartidor') !== null) {
            ...
            cards.forEach((card) => {
                obj.barriles.push({
                    ...
                    id_usuarios_repartidor: $(card).data('id-usuarios-repartidor'),
                    id_despachos: $(card).data('id-despachos')
                });
            });
            data.repartidores.push(obj);
        }
    });
