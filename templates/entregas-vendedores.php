<?php
    ...
    $vendedor = new Usuario($_GET['id']);   
    
  $clientes = Cliente::getAll("WHERE id_usuarios_vendedor='".$vendedor->id."'");
  ...
  $entregas = array();
    ...
    if($modo == "Semanal") {
        $query = "WHERE id_usuarios_vendedor='".$vendedor->id."' AND creada BETWEEN '".$esta_semana->lunes." 00:00:00' AND '".$esta_semana->domingo." 23:59:59'";
    } else {
        $query = "WHERE id_usuarios_vendedor='".$vendedor->id."' AND creada BETWEEN '".$ano."-".$mes."-01 00:00:00' AND '".$ano."-".$mes."-31 23:59:59'";
    }
...
<div class="d-sm-flex align-items-center justify-content-between mb-3">
  <div class="mb-2">
  <div class="mb-2">
    <h1 class="h4 mb-0 text-gray-800"><b><?= $vendedor->nombre; ?></b></h1>
  ...
  <tbody>
    ...
    <tfooter>
      <tr style="border: 1px solid black; background-color: white">
        ...
        <td>
          <?= $vendedor->vendedor_meta_barriles; ?>
        </td>
        <td>
          <?= $totales['Cajas']; ?>
        </td>
        <td>
          <?= $vendedor->vendedor_meta_cajas; ?>
        </td>
...
$(document).on('change','#mes-select', function(e) {
  window.location.href = "./?s=entregas-vendedores&mes=" + $('#mes-select').val() + "&ano=" + $('#ano-select').val() + "&id_clientes=" + id_clientes + "&modo=" + $("#vista-select").val() + "&id=" + id_vendedores;
});
...
$(document).on('change','#lunes-select', function(e) {
  window.location.href = "./?s=entregas-vendedores&lunes=" + $(e.currentTarget).val() + "&id_clientes=" + id_clientes + "&modo=" + $("#vista-select").val() + "&id=" + id_vendedores;
});

$(document).on('change','#clientes-select', function(e) {
  window.location.href = "./?s=entregas-vendedores&lunes=" + lunes + "&id_clientes=" + $(e.currentTarget).val() + "&modo=" + $("#vista-select").val() + "&id=" + id_vendedores;
});

$(document).on('change','#vista-select', function(e) {
  window.location.href = "./?s=entregas-vendedores&lunes=" + lunes + "&id_clientes=" + id_clientes + "&modo=" + $("#vista-select").val() + "&id=" + id_vendedores;
});
