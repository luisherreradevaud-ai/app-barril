<?php
  ...
  $query = "WHERE id_usuarios_vendedor='".$usuario->id."' ORDER BY ".$order_by." ".$order." LIMIT 20 OFFSET ".$offset;
  $clientes = Cliente::getAll($query);
  $clientes_total = Cliente::getAll("WHERE id_usuarios_vendedor='".$usuario->id."'");

  $pages = count($clientes_total) / 20;
...
<div class="d-sm-flex align-items-center justify-content-between mb-3">
  ...
  <div>
    <div>
      <a href="./?s=vendedores-detalle-clientes" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm mb-2"><i class="fas fa-fw fa-plus"></i> Nuevo Cliente</a>
    </div>
  </div>
</div>
<hr />
...
<div class="d-flex justify-content-between mt-4 mb-5">
  <div>
    <b>Mostrando <?= count($clientes); ?> de <?= count($clientes_total); ?></b>
  </div>
  <div>
    <b>P&aacute;gina:</b> 
...
$(document).on('click','.tr-clientes',function(e){
  window.location.href = "./?s=vendedores-detalle-clientes&id=" + $(e.currentTarget).data('idclientes');
});
